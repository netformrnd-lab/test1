// Cloudflare Pages Function → 경로 /push
// Supabase Database Webhook가 새 글(감리일지/현장현황/공지/채팅)을 여기로 POST하면,
// 대상 사용자들의 FCM 토큰을 찾아 Firebase(FCM v1)로 푸시 알림을 발송한다.
//
// 필요한 환경변수(Cloudflare Pages → Settings → Environment variables, 암호로 저장):
...
//   PUSH_WEBHOOK_SECRET    = (아무 비밀문자열; 웹훅 URL ?secret= 또는 헤더 x-webhook-secret 로 검증)
...
export async function onRequestPost(context) {
  ...
  try {
    const plan = await buildPlan(env, table, type, rec, old)   // {title, body, userIds, data}
...
// GET 등은 막기
export async function onRequest() { return json({ ok: false, error: 'POST only' }, 405) }
