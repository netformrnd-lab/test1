...
if ($action === 'prompt') {
    ...
    if ($kind === 'path') {
        /* 문서를 읽고 「신입 온보딩 다섯 단」 을 씁니다.
           한 번에 다섯 단을 다 쓰면 답이 길어서 시간 안에 못 끝냅니다
           (실제로 100초를 넘겨 자주 끊겼습니다).
           그래서 두 번에 나눠 묻습니다 — 화면이 part=1, part=2 로 두 번 부릅니다.
             part 1  여정 · 지킬 것   (분량이 제일 큰 쪽)
             part 2  무엇 · 사람
             part 3  한문단 — 문서가 아니라 「쌓인 여정 전체」 를 보고 씁니다
           part 을 안 주면 예전처럼 한 번에 다 씁니다.                       */
        $part = (int)($b['part'] ?? 0);
        /* 문서 갈래 — 어느 문서가 어느 단을 채우는지 나눕니다.
             plan  기획서·제안서       → 여정
             guide 지침서·매뉴얼·브랜드북 → 지킬 것
           갈래를 모른 채 한 번에 다 뽑으면 한쪽으로 쏠립니다. */
        $doc = trim((string)($b['doc'] ?? 'auto'));
        if ($doc !== 'plan' && $doc !== 'guide') $doc = 'auto';
        $seen = trim((string)($b['여정'] ?? ''));   // part 2 에 넘겨주는 part 1 의 결과

        $head = "당신은 브랜드의 신입 온보딩 문서를 쓰는 사람입니다.\n"
             . "신입이 첫날 이것만 읽고 「이 브랜드에서 내가 어떻게 일하면 되는지」 를\n"
             . "알게 만드는 것이 목적입니다. 회사 소개나 연혁 나열이 아닙니다.\n\n";

        $form = "■ 이 문서가 「기획서 양식」 이면 (제일 흔한 경우)\n"
             . "우리 팀 기획서는 일곱 절로 되어 있습니다.\n"
             . " 머리말: 브랜드 · 기획명 · 작성자 · 작성일 · 한 줄 목적\n"
             . " 1 기획 개요  2 문제 정의  3 조사·레퍼런스 분석  4 타겟·페르소나\n"
             . " 5 기획 내용  6 실행 계획  7 결과·수정 기록\n"
             . "이 양식이면 기획서 한 장이 여정 한 칸입니다. 여러 칸으로 쪼개지 마세요.\n"
             . "한 파일에 시트가 여럿이면 내용이 채워진 시트마다 한 칸입니다.\n"
             . "양식만 있고 아무 칸도 안 채워졌으면 「못읽음」 으로 답하세요.\n\n";

        $cannot = "■ 먼저 판단하세요 — 이 문서로 그것을 쓸 수 있습니까?\n"
             . "브랜드가 무엇을 팔고 · 누구에게 가고 · 어떻게 말하고 · 무엇을 하지 않는지가\n"
             . "하나도 없는 문서면 억지로 쓰지 말고 아래처럼만 답하세요.\n"
             . "{\"못읽음\":\"이 문서는 (무슨 문서)라서 브랜드가 어떻게 일하는지가 적혀 있지 않습니다\"}\n"
             . "기능 개선 내역 · 화면 기획서 · 업무 절차서 · 매뉴얼 · 회의록 · 견적서가\n"
             . "그런 문서입니다.\n\n";

        $rules = "규칙:\n"
             . "- 문서에 없는 사실을 절대 지어내지 마세요. 근거가 없으면 그 줄을 버립니다.\n"
             . "- 날짜를 추측하지 마세요. 문서에 없으면 when 을 비웁니다.\n"
             . "- 실패를 완곡하게 쓰지 마세요. 무엇이 안 됐는지를 그대로 적습니다.\n"
             . "- 「열심히·최대한·많이·편리하게」 대신 문서에 적힌 사실을 쓰세요.\n"
             . "- 기능을 더했다 · 화면을 고쳤다 같은 제품 이야기는 여정이 아닙니다.\n"
             . "  다만 그 결과로 브랜드가 무엇을 팔거나 말하는 방식이 바뀌었다면 넣으세요.\n"

             . "- ★ 사람이 확인할 수 있는 말로만 쓰세요.\n"
             . "  이 글은 신입이 읽고, 읽다가 「이건 무슨 말이지」 싶으면 직접 찾아가\n"
             . "  확인해야 하는 글입니다. 찾아갈 수 없는 표시는 쓰면 안 됩니다.\n"
             . "  · 「§21-C」 「A-3안」 「Rev.2」 「p.12 표 2」 「3절」 처럼 그 문서\n"
             . "    안에서만 통하는 기호·조항·쪽번호를 그것만 달랑 쓰지 마세요.\n"
             . "    꼭 남겨야 하면 문서 이름과 날짜를 같이 적으세요\n"
             . "    (예: 「서비스 제안서 2025.04.03 의 조사 항목」).\n"
             . "  · 줄임말·사내 약칭을 처음 쓸 때는 풀어 쓰고 괄호로 묶으세요\n"
             . "    (예: 「설계감리 10단계(설계 단계에서 도서를 확인하는 절차)」).\n"
             . "  · 무슨 뜻인지 문서에 설명이 없는 기호는 아예 빼세요.\n"
             . "    신입은 그 줄에서 막히고, 물어볼 데도 없습니다.\n"

             . "- 답은 JSON 하나만 쓰세요. 설명·인사말·마크다운 기호를 쓰지 마세요.\n";

        /* ── 여정 · 지킬 것 (part 1) ── */
        $mapA = "기획서를 이렇게 옮깁니다.\n"
             . " 기획명            → title (그리고 plan 에도 같은 값을 넣으세요)\n"
             . " 작성일            → when\n"
             . " 한 줄 목적 · 5절   → body (무엇을 했는지)\n"
             . " 2절 고객 문제·기존 한계 → why (왜 했나)\n"
             . " 7절 실제 결과 + 잘된 점 → result\n"
             . " 7절 문제·실패      → bad\n"
             . " 5절 제외 범위 · 피해야 할 운영 → nogo\n"
             . " 3절 출처·기준일    → src\n"
             . " 7절 상태          → 끝났으면 「해냈다」, 접었으면 「접었다」,\n"
             . "                     방향이 바뀌었으면 「방향을 틀다」, 아직이면 「해봤다」\n"
             . "7절이 통째로 비어 있으면 result 를 비우세요 (아직 하는 중입니다).\n\n"
             . "지킬것은 7절을 기다리지 말고 5절에서 먼저 뽑습니다. 신입이 내일 쓰는 것이라\n"
             . "기획이 아직 진행 중이어도 이 줄들은 있어야 합니다.\n"
             . " 5절 핵심 메시지          → 지킬것 「쓰는 말」\n"
             . " 5절 운영 기준 · 품질 기준 → 지킬것 「지키는 절차」\n"
             . " 5절 피해야 할 운영        → 지킬것 「지키는 절차」 (하지 않는 쪽 그대로)\n"
             . " 5절 제외 범위            → 지킬것 「그 밖」 (nogo 에도 그대로)\n"
             . " 7절 수정 내용·현재 기준의 「앞으로 이렇게 한다」 → 지킬것\n"
             . "이 줄들의 「근거」 에는 모두 이 기획명을 그대로 적으세요.\n"
             . "5절이 표어처럼만 적혀 있으면 (예: 고객 만족) 버리세요. 신입이 그대로\n"
             . "따라 할 수 있는 문장만 지킬것으로 만듭니다.\n\n";

        $chapOnly = "■ 여정만 씁니다\n"
             . " 여정 — 이 브랜드가 걸어온 길. 무엇을 했고 어떻게 됐는지.\n"
             . "   갈래: 출발 · 해봤다 · 방향을 틀다 · 해냈다 · 접었다 · 지금\n"
             . "   시간 순서로. 마지막은 되도록 「지금」 으로 끝내세요.\n"
             . " 이 문서는 기획서입니다. 지킬것은 7절 「수정 내용·현재 기준」 에서\n"
             . " 실제로 바꾸기로 한 것만 만드세요. 5절의 계획은 아직 규칙이 아니라\n"
             . " 그 기획의 내용입니다 — 그것은 여정의 body 로 보냅니다.\n\n";

        $ruleOnly = "■ 지킬 것만 씁니다\n"
             . " 이 문서는 지침서입니다. 「어떻게 일하는지」 가 적혀 있습니다.\n"
             . " 지킬것 — 신입이 내일 당장 쓰는 것. 여기가 이 문서의 전부입니다.\n"
             . "   갈래: 말투 · 쓰는 말 · 안 쓰는 말 · 지키는 절차 · 그 밖\n"
             . "   각 줄에 why(왜 그런지 한 줄)를 되도록 채우세요.\n"
             . "   규칙만 있으면 신입이 안 지킵니다. 왜 생겼는지가 붙어야 납득합니다.\n"
             . "   신입이 그대로 따라 할 수 있는 문장만 만드세요. 표어는 버립니다.\n"
             . " 여정은 빈 목록으로 두세요. 지침서는 「무엇을 했다」 가 아니라\n"
             . " 「이렇게 한다」 입니다. 억지로 여정을 만들면 길이 아니라 목록이 됩니다.\n"
             . " 다만 「예전에는 이랬는데 이렇게 바꿨다」 가 날짜와 함께 적혀 있으면\n"
             . " 그것만 여정 한 칸으로 만드세요.\n"
             . "\n■ 조항 번호는 풀어서 씁니다 (제일 중요)\n"
             . "지침서는 대개 「§4-B 판정 어휘로 쓴다」 처럼 자기 문서 안의 조항을\n"
             . "가리킵니다. 신입은 그 번호를 보고 아무것도 확인할 수 없습니다.\n"
             . "번호가 가리키는 내용이 같은 문서 안에 있으니, 그 내용을 찾아\n"
             . "그 자리에 풀어 넣고 번호는 버리세요.\n"
             . " ✕ 판정은 §4-B 어휘로만 씁니다\n"
             . " ○ 판정은 미판정 · 적합 · 조치 요청 · 조치 완료 · 부적합\n"
             . "   다섯 가지로만 씁니다\n"
             . " ✕ §22-C 위험 카피를 쓰지 않습니다\n"
             . " ○ 「점검합니다 · 검사합니다 · 상주 · 매일 확인」 을 쓰지 않습니다 —\n"
             . "   계약 의무가 되기 때문입니다\n"
             . "같은 문서 안에 그 조항의 내용이 없으면 그 줄을 아예 버리세요.\n"
             . "APTSQ-EDU-001 · Rev.1.0 같은 문서 코드도 마찬가지입니다.\n"
             . "부르는 이름이 따로 있으면 그 이름으로 (예: 감리실무 교육자료).\n\n"

             . "■ 대조표는 한 줄이 규칙 하나입니다\n"
             . "「원래 표현 → 교정 표현」, 「금지 → 대체」, 「위험 → 안전」 같은 표가\n"
             . "있으면 한 줄씩 규칙으로 만드세요. 실제 말을 그대로 남깁니다.\n"
             . " ○ 「사고를 막습니다」 대신 「확인하고 기록합니다」 를 씁니다\n"
             . " ○ 「점검합니다」 대신 「확인합니다」 를 씁니다 — 점검은 계약 의무가 됩니다\n"
             . "줄이 너무 많으면 비슷한 것끼리 묶어 한 줄로 쓰되, 바꿔 쓸 말은\n"
             . "빠짐없이 적으세요. 신입이 그 줄만 보고 고쳐 쓸 수 있어야 합니다.\n\n"

             . " 「근거」 — 아래에 이미 쌓여 있는 여정 목록을 같이 보냅니다.\n"
             . "   이 규칙이 그중 어느 일에서 나왔는지 알겠으면, 그 여정의 제목을\n"
             . "   글자 그대로 「근거」 에 적으세요. 화면이 규칙 옆에 그 일을 붙여\n"
             . "   보여줍니다. 규칙만 있으면 신입이 안 지킵니다.\n"
             . "   억지로 걸지는 마세요 — 맞는 것이 없으면 빈 문자열로 둡니다.\n\n";

        $secA = "■ 두 가지를 씁니다\n"
             . " 여정 — 이 브랜드가 걸어온 길. 무엇을 했고 어떻게 됐는지.\n"
             . "   갈래: 출발 · 해봤다 · 방향을 틀다 · 해냈다 · 접었다 · 지금\n"
             . "   시간 순서로. 마지막은 되도록 「지금」 으로 끝내세요.\n"
             . " 지킬것 — 여기서 일할 때 지킬 것. 신입이 내일 당장 쓰는 것입니다.\n"
             . "   갈래: 말투 · 쓰는 말 · 안 쓰는 말 · 지키는 절차 · 그 밖\n"
             . "   각 줄에 why(왜 그런지 한 줄)를 되도록 채우세요. 규칙만 있으면 안 지킵니다.\n"
             . " 실물 — 「이렇게 쓰지 말고 이렇게」 한 쌍. 규칙이 「하지 마라」 라면\n"
             . "   이건 「그럼 뭐라고 쓰나」 입니다. 문서에 좋은 예 / 나쁜 예, 대조표,\n"
             . "   「A 가 아니라 B」 처럼 적힌 데가 있으면 전부 여기로 가져오세요.\n"
             . "   옳은 쪽만 남기지 마세요 — 잘못된 쪽이 있어야 지금 쓰던 게\n"
             . "   왜 문제였는지를 압니다.\n\n"

             . "■ 여정에는 반드시 「결과」 가 있어야 합니다\n"
             . "「무엇을 했다」 만 있고 「그래서 어떻게 됐다」 가 없으면 여정이 아니라\n"
             . "그냥 목록입니다. result 에 어떻게 됐는지를 적으세요. 숫자가 있으면 숫자로.\n"
             . "문서에 결과가 없으면 result 를 비워 둡니다 (「아직 하는 중」 으로 나옵니다).\n"
             . "지어내지 마세요.\n\n";

        $shapeA = "{\"여정\":[{\"when\":\"2022\",\"kind\":\"출발\",\"title\":\"\",\"plan\":\"\",\n"
             . "         \"body\":\"\",\"result\":\"\",\"before\":\"\",\"after\":\"\",\"why\":\"\",\n"
             . "         \"bad\":\"\",\"nogo\":\"\",\"src\":\"\"}],\n"
             . " \"지킬것\":[{\"kind\":\"안 쓰는 말\",\"text\":\"\",\"why\":\"\",\"어기면\":\"\",\n"
             . "             \"어디\":\"\",\"근거\":\"\",\"잠정\":false,\"확정조건\":\"\"}],\n"
             . " \"실물\":[{\"title\":\"\",\"말고\":\"\",\"이렇게\":\"\",\"왜\":\"\",\"url\":\"\"}]}\n\n"
             . "실물의 「title」 — 무엇을 쓸 때인지 한 마디로. 예) 블로그 첫 문장 ·\n"
             . "감리 사례 카드 사진 · 제안서 첫 장. 어느 자리에서 헷갈리는지를 적습니다.\n"
             . "실물의 「말고」 — 흔히 이렇게 쓰는 것. 문서에 나쁜 예가 있으면 글자\n"
             . "그대로. 없으면 문서가 「하지 마라」 고 한 것을 실제 문장으로 만들어\n"
             . "적으세요 (규칙 문장이 아니라, 그 규칙을 어긴 <<실제 결과물>> 로).\n"
             . "실물의 「이렇게」 — 옳게 쓴 실물 한 줄. 설명이 아니라 그대로 쓸 수\n"
             . "있는 문장 · 구성이어야 합니다.\n"
             . "실물의 「왜」 — 왜 그 쪽이 맞는지. 이게 있어야 다음에 처음 보는\n"
             . "자리에서도 같은 판단을 합니다.\n"
             . "둘 다 없으면 그 줄은 만들지 마세요. 지어내지 않습니다.\n\n"
             . "지킬것의 「어디」 — 이 규칙을 어디에서 쓰는지 한 단어로 적습니다.\n"
             . "문서에 적힌 말을 그대로 쓰세요 (블로그 · 영상 · 카페 · 촬영 · 제안서 …).\n"
             . "어디서 일하든 지켜야 하는 것이면 「전체」 라고 쓰세요.\n"
             . "규칙이 쌓이면 채널 것이 한데 섞여 길어집니다. 블로그 원고를 쓰는\n"
             . "사람에게 촬영 규칙은 지금 필요 없으므로, 골라 볼 수 있게 하려는 것입니다.\n\n"
             . "여정의 칸:\n"
             . " when 그 때 · title 한 줄 제목 · body 한 일 · result 결과\n"
             . " before/after 무엇이 무엇으로 바뀌었나 (방향을 튼 것에만)\n"
             . " why 왜 그런 결과가 났나 · bad ✕ 안 된 것 · nogo ✕ 하지 않기로 한 것\n"
             . " src 어느 자료 · 언제 일인가 — 사람이 그것만 보고 찾아갈 수 있게\n"
             . "     문서 이름과 날짜로 적습니다 (예: 「서비스 제안서 2025.04.03」).\n"
             . "     쪽번호·조항 기호만 달랑 적지 마세요.\n"
             . "title 과 body 는 반드시 채우세요. 둘 다 못 채울 것은 만들지 마세요.\n\n"
             . "지킬것의 「근거」 — 그 줄이 왜 생겼는지가 여정에 있으면, 그 여정의\n"
             . "title 을 글자 그대로 옮겨 적으세요. 화면이 그 여정을 규칙 옆에 붙여\n"
             . "보여줍니다. 규칙만 있으면 신입이 안 지키고, 왜 생겼는지가 붙어야\n"
             . "납득합니다. 해당하는 여정이 없으면 빈 문자열로 두세요.\n"
             . "- 신입이 제일 알고 싶은 것은 「우리가 왜 이건 안 하는지」 입니다.\n"
             . "  근거가 있으면 안 쓰는 말 · bad · nogo 를 반드시 채우세요.\n\n"

             . "지킬것의 「어기면」 — 그 줄을 어겼을 때 <<무슨 일이 벌어지는지>> 를 씁니다.\n"
             . "★ why 는 원칙이고 「어기면」 은 장면입니다. 둘은 다릅니다.\n"
             . "  경력자는 「안전보건조치 의무자는 시공자입니다」 한 줄에 담긴 무게를\n"
             . "  알지만 신입은 모릅니다. 이해 없이 외운 금지어는 응용이 안 되고,\n"
             . "  응용이 안 되면 목록에 없는 표현에서 반드시 사고가 납니다.\n"
             . "  그래서 신입이 그림을 그릴 수 있게, <<누가 · 언제 · 무엇을 하게 되는지>> 를\n"
             . "  한두 문장으로 씁니다.\n"
             . "  (나쁜 예) 「법적 책임이 발생합니다」 — 아무 장면도 안 그려집니다.\n"
             . "  (좋은 예) 「현장에서 사고가 나면, 우리가 원고에 쓴 '막는다' 한 줄이\n"
             . "            우리를 안전 의무자로 만듭니다. 우리 계약에 없는 책임입니다.」\n"
             . "  문서에 그런 대목이 없으면 지어내지 말고 비워 두세요.\n\n"

             . "지킬것의 「잠정」 · 「확정조건」 — 아직 확정이 아닌 규칙을 가려냅니다.\n"
             . "★ 문서가 「…확인 전까지는 잠정」 「가계약 기준」 「추후 확정」 「TBD」\n"
             . "  처럼 못박아 둔 대목이 있으면, 그 아래에 딸린 규칙은 모두\n"
             . "  잠정: true 로 하고 확정조건에 <<무엇이 정해지면 풀리는지>> 를 씁니다.\n"
             . "  (예: 확정조건 「계약 유형이 확인되면」)\n"
             . "  이 표시가 빠지면 신입이 잠정 기준을 확정으로 알고 외부 문서에\n"
             . "  그대로 씁니다. 한 절 전체가 잠정이면 그 절의 규칙을 하나도 빠짐없이\n"
             . "  잠정으로 표시하세요. 확정이면 잠정: false 입니다.\n\n";

        /* ── 무엇 · 사람 (part 2) ── */
        $secB = "■ 세 가지를 씁니다\n"
             . " 무엇 — 이 브랜드는 뭘 하나. 다섯 칸입니다.\n"
             . "   ★ 신입은 아직 이 일이 무슨 일인지, 누가 우리에게 돈을 주는지,\n"
             . "     계약이 어디까지인지 모릅니다. 규칙보다 이것이 먼저입니다.\n"
             . "     문서 안에 이 사실이 표 각주나 괄호로 한 번만 나와도 끌어올려 여기 쓰세요.\n"
             . "   한문단 — 두세 문장. 꾸미지 말고 하는 일을 그대로.\n"
             . "     그 기획 한 건의 목적이 아니라 브랜드가 무엇을 하는 곳인지를 씁니다.\n"
             . "   누가돈을주나 — 우리에게 일을 맡기고 비용을 내는 쪽.\n"
             . "     실제로 쓰는 사람이 다르면 둘 다 씁니다.\n"
             . "   우리역할 — 이 일에서 우리가 맡는 자리. 문서가 역할을 몇 가지로\n"
             . "     나눠 두었으면 그 이름을 그대로 씁니다. 줄바꿈으로 나눕니다.\n"
             . "   계약범위 — 어떤 계약인지 · 어디까지가 우리 몫인지.\n"
             . "     상주인지 비상주인지, 몇 주에 몇 번인지 같은 조건을 빠뜨리지 마세요.\n"
             . "     문서가 「확인 전까지 잠정」 이라고 했으면 그 말도 같이 씁니다.\n"
             . "   하지않는것 — 남이 우리 일로 오해하기 쉬운데 우리 몫이 아닌 것.\n"
             . "   각 칸은 문서에 근거가 없으면 비워 둡니다. 지어내지 마세요.\n"
             . "   (머리말 브랜드 · 1절 목적 · 5절 핵심 가치·제공 범위를 봅니다)\n"
             . " 사람 — 막히면 누구에게 물어보나. <<사람 이름>>만 씁니다.\n"
             . "   ★ 문서에 사람 이름이 없으면 반드시 빈 목록 [] 로 답하세요.\n"
             . "     「브랜드·마케팅팀」 같은 팀 이름, 「담당자 미지정」, 「확인 필요」,\n"
             . "     「추후 지정」, 「별도 안내」 는 <<절대>> 쓰지 마세요. 그런 말을 채우면\n"
             . "     빈칸이 채워진 것처럼 보여서 아무도 진짜 이름을 넣지 않습니다.\n"
             . "     비워 두면 화면이 「아직 없다」 고 짚어 주고 사람이 채웁니다.\n"
             . "   role 에는 직함이 아니라 <<언제 말을 걸면 되는지>> 를 씁니다.\n"
             . "     (예: 「원고 판정이 헷갈릴 때」 — 「팀장」 이 아니라)\n"
             . "   (머리말 작성자·검토자 · 5절 담당자를 봅니다)\n"
             . " 얼굴 — 이 브랜드가 어떤 얼굴인가. 원고를 쓰는 사람이 매번 묻는 것들입니다.\n"
             . "   ★ 아래 칸은 <<문서에 적힌 것만>> 씁니다. 근거가 없으면 빈 문자열 \"\" 나\n"
             . "     빈 목록 [] 로 두세요. 그럴듯하게 지어내면 아무도 진짜를 채우지 않습니다.\n"
             . "   슬로건 — 밖에 내보내는 한 줄. 표어·캐치프레이즈·태그라인.\n"
             . "   성격 — 이 브랜드가 사람이라면 어떤 사람인가. 한 문장으로.\n"
             . "     형용사만 늘어놓지 말고 「~하는 사람」 으로 씁니다.\n"
             . "   철학 — 왜 이 일을 하나 · 무엇을 믿나. 판단이 갈릴 때 돌아오는 기준.\n"
             . "     철학이 여러 갈래로 적혀 있으면 아래 네 칸에 나눠 담습니다.\n"
             . "     존재이유 — 이 브랜드가 왜 있어야 하나 · 없으면 무엇이 안 되나.\n"
             . "     기술철학 — 기술을 어떻게 다루나 · 무엇을 믿고 무엇을 안 하나.\n"
             . "     해야할일 — 그래서 실제로 무엇을 하나.\n"
             . "     철학목표 — 어디까지 가려고 하나.\n"
             . "     한 갈래만 적혀 있으면 그 칸만 채우고 나머지는 빈 문자열로 둡니다.\n"
             . "   역할 — <<상대마다>> 우리가 무엇인가. 문서가 「시공사에게는 ~」\n"
             . "     처럼 상대를 나눠 적었으면 who 에 상대, what 에 그 상대에게\n"
             . "     우리가 무엇인지를 씁니다. 상대 구분이 없으면 [] 로 두고\n"
             . "     우리역할 칸에만 씁니다.\n"
             . "   타겟 — 누구에게 말하나. 한 줄.\n"
             . "   톤앤매너 — 어떤 말투로 쓰나(언어 톤). 존댓말인지 · 감탄사를 쓰는지 ·\n"
             . "     얼마나 딱딱한지. 색·글꼴은 여기 쓰지 말고 아래 칸에 나눠 씁니다.\n"
             . "   표현키워드 — 늘 붙이는 <<낱말>>만. 「·」 로 이어 붙입니다.\n"
             . "     (예: 정중함 · 기술적 · 데이터 기반) 문장은 여기 쓰지 마세요.\n"
             . "   브랜드어조 — 무엇으로 말하나 한 줄.\n"
             . "     (예: 감성이 아니라 데이터로 말한다)\n"
             . "   페르소나 — 실제로 있을 법한 사람. 문서가 고객 유형을 나눠 두었으면\n"
             . "     그것을 씁니다. 없으면 []. name 은 「40대 아파트 관리소장」 처럼.\n"
             . "   색 — 지정색이 적혀 있으면. hex 는 #RRGGBB 로만. 색이름만 있고\n"
             . "     색값이 없으면 hex 를 비우고 name 에 그 이름을 씁니다.\n"
             . "   글꼴 — 지정 서체가 적혀 있으면. use 에는 어디에 쓰는지.\n"
             . "   용어 — <<한 단어짜리>> 쓰는 말 · 안 쓰는 말. 안 쓰는 말이면\n"
             . "     안쓴다 를 true 로 하고 instead 에 대신 쓸 말을 씁니다.\n"
             . "     설명이 길어서 문장이 되는 것은 여기 말고 part 1 의 지킬것으로 갑니다.\n"
             . "   해목표 — 연도별 목표가 적혀 있으면 해마다 한 칸씩.\n"
             . "     그 해 안에서 분기·월·항목으로 갈라 적혀 있으면 세부 에 나눠\n"
             . "     담습니다. 때 는 「1분기」 「3월」 처럼 적힌 것이 있을 때만,\n"
             . "     무엇 은 그 하나로 한 일, 됨 은 끝났다고 적혀 있으면 true.\n"
             . "     year 는 네 자리 · goal 은 그 해에 무엇을 · why 는 무엇을 위해 ·\n"
             . "     result 는 어떻게 됐는지(적혀 있을 때만).\n"
             . "   (머리말 브랜드 · 2절 브랜드 정체성 · 3절 타겟 · 4절 표현 기준을 봅니다)\n"
             . "\n";
        ...
        $shapeB = "{\"무엇\":\"\",\n"
             ...
             . " \"표현키워드\":\"\", \"브랜드어조\":\"\",\n"
             . " \"역할\":[{\"who\":\"\",\"what\":\"\"}],\n"
             . " \"페르소나\":[{\"name\":\"\",\"age\":\"\",\"who\":\"\",\"want\":\"\",\n"
             . "               \"pain\":\"\",\"where\":\"\"}],\n"
             . " \"색\":[{\"hex\":\"\",\"name\":\"\",\"use\":\"\"}],\n"
             . " \"글꼴\":[{\"name\":\"\",\"use\":\"\"}],\n"
             . " \"용어\":[{\"term\":\"\",\"안쓴다\":false,\"instead\":\"\",\"why\":\"\"}],\n"
             . " \"해목표\":[{\"year\":\"\",\"goal\":\"\",\"why\":\"\",\"result\":\"\",\n"
             . "             \"세부\":[{\"때\":\"\",\"무엇\":\"\",\"됨\":false}]}],\n"
             . " \"사람\":[{\"name\":\"\",\"role\":\"\",\"where\":\"\"}]}\n\n";

        /* 문서가 길면 답도 길어지고, 길어지면 시간 안에 못 끝냅니다.
           빠뜨린 것은 같은 문서를 한 번 더 올리면 빈 칸만 이어서 채워지므로,
           한 번에 받는 양을 줄이는 쪽이 낫습니다. */
        $long = function_exists('mb_strlen') && mb_strlen($book, 'UTF-8') > 8000;
        $brief = $long
            ? "■ 문서가 깁니다 — 짧게 쓰세요\n"
              . "한 칸은 문장 두 개를 넘기지 마세요. 길게 쓰면 시간 안에 못 끝냅니다.\n"
              . "제일 중요한 것부터 쓰고, 덜 중요한 것은 버리세요.\n"
              . "여정은 많아야 6칸 · 지킬것은 많아야 8줄.\n\n"
            : '';

        /* ── 조항 번호 고치기 (part 4) ──
           1/2 를 아무리 일러도 모델이 「§4-B 어휘로」 같은 줄을 가끔 남깁니다.
           그때 그 줄들만 따로 보내 번호를 내용으로 바꿔 받습니다.
           보내는 것도 받는 것도 짧아서 빠르고, 실패해도 원래 줄이 남습니다. */
        if ($part === 4) {
            $sys = "아래 「고칠 줄」 들에는 그 문서 안에서만 통하는 조항 번호나\n"
                 . "문서 코드가 남아 있습니다 (§4-B · 4-B · §21 · APTSQ-EDU-001 · Rev.1.0 …).\n"
                 . "읽는 사람은 그 번호를 보고 아무것도 확인할 수 없습니다.\n\n"
                 . "문서에서 그 번호가 가리키는 내용을 찾아, 번호를 그 내용으로\n"
                 . "바꿔 같은 줄을 다시 쓰세요.\n"
                 . " 예) 「판정은 §4-B 어휘로만 씁니다」\n"
                 . "   → 「판정은 미판정 · 적합 · 조치 요청 · 조치 완료 · 부적합\n"
                 . "      다섯 가지로만 씁니다」\n"
                 . " 예) 「§22-C 위험 카피를 쓰지 않습니다」\n"
                 . "   → 「「점검합니다 · 검사합니다 · 상주 · 매일 확인」 을 쓰지 않습니다」\n\n"
                 . "규칙:\n"
                 . "- 번호가 가리키는 내용만 바꾸고 나머지 말은 그대로 두세요.\n"
                 . "- 문서에서 뜻을 못 찾으면 그 번호를 통째로 빼고 나머지만 남기세요.\n"
                 . "  (「§4-B 에 따라 판정합니다」 → 「판정합니다」)\n"
                 . "- 새 사실을 지어내지 마세요. 문서에 있는 말만 씁니다.\n"
                 . "- 조항 번호·문서 코드를 결과에 절대 남기지 마세요.\n"
                 . "- id 는 받은 그대로 돌려주세요. 줄을 빼먹지 마세요.\n\n"
                 . "답은 이 모양의 JSON 하나만 쓰세요.\n"
                 . "{\"고친줄\":[{\"id\":\"\",\"text\":\"\"}]}\n";
            $user = "--- 고칠 줄 ---\n" . trim((string)($b['줄'] ?? ''))
                  . "\n\n--- 문서 ---\n" . $book;
        } else
        ...
        /* ── 한 문단 (part 3) ──
           문서 한 장을 보고 브랜드 전체를 요약하면 그 문서 이야기만 적힙니다.
           그래서 여기서는 문서가 아니라 「지금까지 쌓인 여정 전체」 를 보고 씁니다. */
        if ($part === 3) {
            $sys = "당신은 브랜드가 걸어온 길을 한 문단으로 쓰는 사람입니다.\n"
                 . "아래는 이 브랜드의 여정 전체입니다. 이것만 보고 세네 문장으로 쓰세요.\n"
                 . "맨 처음부터 오늘까지가 한 줄기로 읽혀야 합니다 — 요즘 한 일만 적으면\n"
                 . "그건 걸어온 길이 아니라 지금 걸어가는 길입니다.\n\n"
                 . "· 출발 → 꺾인 데 → 지금 순서로 씁니다.\n"
                 . "· 신입이 이 한 문단만 읽어도 「이 브랜드가 어떻게 여기까지 왔는지」 를\n"
                 . "  알게 되어야 합니다.\n"
                 . "· 기능을 늘렸다 · 화면을 고쳤다 · 앱을 편하게 만들었다 같은 제품 이야기는\n"
                 . "  쓰지 마세요. 브랜드가 무엇을 팔고 · 누구에게 가고 · 어떻게 말하는지가\n"
                 . "  어떻게 바뀌었는지를 씁니다.\n"
                 . "· 마디 하나하나를 차례로 옮겨 적지 마세요. 그건 목록이지 한 문단이 아닙니다.\n"
                 . "  꺾인 지점만 남기고 나머지는 버립니다.\n"
                 . "· ★ 가 붙은 마디가 이 브랜드의 줄기입니다 (출발 · 방향을 틀다 · 접었다 · 지금).\n"
                 . "  ★ 마디로 뼈대를 세우세요. ★ 가 없는 마디는 그 사이에 한 일이라\n"
                 . "  대부분 버립니다 — 줄기를 설명하는 데 꼭 필요할 때만 한 마디 보탭니다.\n"
                 . "· ★ 마디가 하나도 없으면 한 문단을 쓸 수 없습니다. 빈 문자열로 답하세요.\n"
                 . "· 여정에 없는 사실을 지어내지 마세요.\n"
                 . "· 마디가 둘 이하면 한 문단을 쓸 수 없습니다. 빈 문자열로 답하세요.\n\n"
                 . "답은 이 모양의 JSON 하나만 쓰세요.\n"
                 . "{\"한문단\":\"\"}\n";
            $user = ($brand !== '' ? "브랜드: $brand\n\n" : '')
                  . "--- 걸어온 길 ---\n" . $book;
        } else if ($part === 1 && $doc === 'guide') {
            $sys = $head . $cannot . $brief . $ruleOnly
                 . "답은 아래 모양의 JSON 하나만 쓰세요.\n" . $shapeA . $rules;
        } else if ($part === 1 && $doc === 'plan') {
            $sys = $head . $form . $cannot . $brief . $chapOnly . $mapA
                 . "답은 아래 모양의 JSON 하나만 쓰세요.\n" . $shapeA . $rules;
        } else if ($part === 1) {
            $sys = $head . $form . $cannot . $brief . $secA . $mapA
                 . "답은 아래 모양의 JSON 하나만 쓰세요.\n" . $shapeA . $rules;
        } else if ($part === 2) {
            $sys = $head . ($doc === 'guide' ? '' : $form) . $brief . $secB
                 . "답은 아래 모양의 JSON 하나만 쓰세요.\n" . $shapeB . $rules
                 . "- 이미 뽑아 둔 여정이 아래에 있습니다. 여정을 다시 쓰지 마세요.\n";
        } else {
            $sys = $head . $form . $cannot . $brief . $secA . $mapA . $secB
                 . "답은 아래 두 모양을 합친 JSON 하나로 쓰세요.\n" . $shapeA . $shapeB . $rules;
        }

        $user = ($brand !== '' ? "브랜드: $brand\n\n" : '')
              . ($seen !== '' && ($part === 2 || ($part === 1 && $doc === 'guide'))
                  ? "--- 이미 쌓여 있는 여정 (근거로 걸 수 있는 것들) ---\n" . $seen . "\n\n" : '')
              . "--- 문서 ---\n" . $book;
    } else if ($kind === 'hist') {
        // 기획서 문서를 읽어 연혁 라인의 마디를 뽑습니다 (JSON 으로만 답합니다)
        $sys = "당신은 브랜드의 「연혁 라인」을 만드는 사람입니다.\n"
             . "받은 문서를 읽고, 라인에 찍을 마디를 뽑아 JSON 으로만 답하세요.\n\n"
             . "마디는 다음 다섯 갈래 중 하나입니다.\n"
             . " · 사건       — 언제 무슨 일이 있었나\n"
             . " · 바뀐 것    — 무엇이 무엇으로 바뀌었나 (전 → 후 → 왜)\n"
             . " · 폐기 표현  — 그만 쓰기로 한 말 (폐기 문장 → 이유 → 지금 표현)\n"
             . " · 실패       — 반려·폐기된 것 (무엇이 안 됐나가 먼저)\n"
             . " · 자료 없음  — 자료가 남아 있지 않은 구간\n\n"
             . "축(axis)은 다음 중 하나이거나 빈 문자열입니다.\n"
             . " what(무엇을 하는가) · who(누구에게) · say(어떻게 말하는가)\n"
             . " where(어디서 만나는가) · keep(무엇을 지키는가)\n\n"
             . "그리고 「지금 이 브랜드가 어떤 곳인지」 도 문서에서 읽어냅니다.\n"
             . "칸은 아홉 개입니다.\n"
             . " what  지금 파는 것        rival 경쟁사와 다른 점\n"
             . " who   핵심 고객           tone  말투\n"
             . " words 쓰는 말 / 안 쓰는 말 where 지금 쓰는 채널\n"
             . " value 약속 (핵심 가치)     nogo  하지 않는 것\n"
             . " why   왜 만들었나\n\n"
             . "답은 아래 모양의 JSON 하나만 쓰세요. 설명·인사말·마크다운 기호를 쓰지 마세요.\n"
             . "{\"한줄\":\"\",\n"
             . " \"지금\":{\"what\":\"\",\"rival\":\"\",\"who\":\"\",\"tone\":\"\",\"words\":\"\",\n"
             . "         \"where\":\"\",\"value\":\"\",\"nogo\":\"\",\"why\":\"\"},\n"
             . " \"마디\":[{\"date\":\"2025.04.03\",\"kind\":\"바뀐 것\",\"axis\":\"what\",\n"
             . " \"title\":\"\",\"body\":\"\",\"before\":\"\",\"after\":\"\",\"why\":\"\",\"fix\":\"\",\n"
             . " \"docAt\":\"3절\",\"guess\":false}]}\n\n"
             . "칸을 갈래에 맞게 채우세요.\n"
             . " · 사건       : title(무엇) · body(내용)\n"
             . " · 바뀐 것    : before(전) · after(후) · why(왜)\n"
             . " · 폐기 표현  : before(폐기 문장 원문) · why(이유) · after(지금 표현)\n"
             . " · 실패       : title(무엇) · body(안 된 것) · before(했던 것) · why(왜) · fix(바꾼 것)\n"
             . " · 자료 없음  : date(구간) · title(왜 없나) · body(어디서 확인)\n"
             . "쓰지 않는 칸은 빈 문자열로 두세요.\n\n"
             . "규칙:\n"
             . "- 문서에 없는 사실을 절대 지어내지 마세요. 없으면 그 마디를 만들지 마세요.\n"
             . "- 문서에 날짜가 없으면 date 를 빈 문자열로 두세요. 날짜를 추측하지 마세요.\n"
             . "- 문서가 추측·추정으로 쓴 대목이면 guess 를 true 로 하세요.\n"
             . "- docAt 에는 그 내용이 문서의 어디에 있었는지 적으세요 (예: 3절, p.12, 표 2).\n"
             . "  모르면 빈 문자열로 두세요.\n"
             . "- 실패는 완곡하게 쓰지 마세요. 무엇이 안 됐는지를 그대로 적으세요.\n"
             . "- 「열심히·최대한·많이」 같은 막연한 말 대신 문서에 적힌 사실을 쓰세요.\n"
             . "- 마디는 많아야 40개까지만 뽑으세요. 같은 내용을 두 번 넣지 마세요.\n"
             . "- 문서에서 마디를 하나도 못 찾으면 마디를 빈 목록으로 두세요.\n\n"
             . "「한줄」·「지금」 규칙:\n"
             . "- 한줄 은 이 브랜드가 무엇을 하는 곳인지 한 문장입니다. 신입이 첫 줄에서\n"
             . "  답을 얻는 자리이므로, 꾸미지 말고 하는 일을 그대로 쓰세요.\n"
             . "- 지금 의 각 칸은 문서에 적힌 「현재」 를 씁니다. 과거 이야기나 바뀌기 전\n"
             . "  이야기를 여기 넣지 마세요 — 그런 것은 마디로 보내세요.\n"
             . "- 문서에 근거가 없는 칸은 빈 문자열로 두세요. 절대 지어내지 마세요.\n"
             . "- words 는 「쓰는 말: … / 안 쓰는 말: …」 처럼 두 쪽을 다 쓰세요.\n"
             . "- 한 칸은 길어야 세 문장입니다. 표어처럼 쓰지 말고 사실로 쓰세요.";
        $user = ($brand !== '' ? "브랜드: $brand\n\n" : '') . "--- 기획서 문서 ---\n" . $book;
    } else if ($kind === 'growth') {
